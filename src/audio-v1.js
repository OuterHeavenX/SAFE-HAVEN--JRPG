...
(()=>{
 ...
 function scene(){if(g.mode==='battle')return'battle';if(g.mode!=='world')return'town';if(g.s?.map==='town'||g.s?.map==='home')return'town';if(g.s?.map==='cave')return'cave';return'world';}
 ...
 setInterval(()=>{const sc=scene();if(sc!==lastScene)restartTimer();const m=g.battle?.message||'';if(m&&m!==lastBattleMessage){lastBattleMessage=m;if(/CRITICAL/i.test(m)){tone(midi(84),.16,'square',.42);noise(.06,.12);}else if(/hits|attacks for/i.test(m)){tone(midi(43),.09,'sawtooth',.22);noise(.045,.09);}else if(/restores|heal/i.test(m)){tone(midi(72),.14,'sine',.24);tone(midi(76),.18,'sine',.18,.05);}else if(/READY/i.test(m)){tone(midi(76),.08,'triangle',.18);}}
