...
(()=>{
 ...
 function sfx(key){if(!unlocked)return;const k=String(key||'');if(/cancel|back/i.test(k))tone(midi(52),.07,'square',.13);else if(/move|select/i.test(k))tone(midi(67),.045,'triangle',.08);else if(/save|savePoint/i.test(k)){tone(midi(72),.16,'sine',.2);tone(midi(79),.24,'sine',.17,.08)}else if(/chest|itemFound/i.test(k)){tone(midi(76),.1,'square',.18);tone(midi(84),.22,'triangle',.2,.08)}else if(/inn/i.test(k)){tone(midi(60),.2,'sine',.14);tone(midi(64),.25,'sine',.12,.1)}else if(/attack|hit/i.test(k)){tone(midi(43),.08,'sawtooth',.2);noise(.04,.07)}else tone(midi(72),.06,'square',.1)}
 ...
 addEventListener('safehaven-audio',e=>sfx(e.detail?.key));
 document.addEventListener('pointerdown',e=>{if(!unlocked||e.pointerType==='touch'||e.pointerType==='pen')return;const b=e.target?.closest?.('button');if(!b)return;sfx(/BACK|CANCEL|NO|FLEE/i.test(b.textContent||'')?'cancel':'confirm')},true);
 window.SafeHavenAudio={unlock,play:sfx,setVolume:v=>{if(master)master.gain.value=Math.max(0,Math.min(.15,Number(v)||0));}};
 restartTimer();
})();
