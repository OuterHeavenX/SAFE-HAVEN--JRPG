...
(()=>{
 ...
 proto.enemyTurn=function(){const p=this.g?.s?.player;if(!p)return base.call(this);C.ensureState(this.g.s);const before=p.hp;base.call(this);if(this.done||p.hp<=0)return;
   ...
   for(const id of ['poison','blind','silence','sleep','slow','haste','berserk'])if(p.statuses[id]?.turns!=null){p.statuses[id].turns--;if(p.statuses[id].turns<=0){delete p.statuses[id];if(id==='poison')this.message+=' Poison wears off.'}}
