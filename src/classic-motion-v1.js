...
(()=>{
 ...
 function frame(t){const b=g.battle;if(g.mode==='battle'&&b){const st=b.visualState||'idle';if(st!==prevState){prevState=st;stateAt=t}const age=(t-stateAt)/1000;ctx.save();
   if(st==='attack'&&age<.42){const p=Math.min(1,age/.18),x=745-(150*p);ctx.globalAlpha=Math.max(0,.65-age);ctx.strokeStyle='#ffe8a0';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(745,238);ctx.lineTo(x,238);ctx.stroke();ring(x,238,18+age*30,'#ffe8a0',.7)}
   else if(st==='cast'&&age<.7){for(let i=0;i<4;i++)ring(745,235,20+i*10+age*12,['#8bd8ff','#ffd878','#c59cff','#8ef0aa'][i],.45)}
