...
function back(){const b=btn('B  BACK',()=>g.menuInput('cancel'),'dt-back');root.appendChild(b);}
...
function simpleList(){const page=g.menu.page,box=document.createElement('div');box.className='dt-simple-list';let rows=[];
 if(page==='items')rows=Object.entries(g.s.inventory).filter(([,n])=>n>0).map(([id,n])=>({label:`${SH.DATA.items[id]?.name||id} ×${n}`,action:()=>{if(id==='potion'){g.s.inventory.potion--;g.s.player.hp=Math.min(g.s.player.maxHp,g.s.player.hp+60);g.notice='Potion used.';g.noticeT=1.2;render();}}}));
...
function jobs(){const box=document.createElement('div');box.className='dt-job-list';JOBS.forEach((j,i)=>{const unlocked=j.unlock(),active=g.s.player.job===j.id;b=document.createElement('button');b.type='button';b.className=unlocked?'':'locked';b.innerHTML=`${active?'◆ ':''}${j.name}<small>${active?'CURRENT JOB':unlocked?'Tap to select':`LOCKED — ${j.cond}`}</small>`;b.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();g.menu.index=i;if(!unlocked){g.notice=`Locked: ${j.cond}`;g.noticeT=1.6;render();return;}if(!active){g.s.player.job=j.id;g.notice=`Job changed to ${j.name}!`;g.noticeT=1.6;SH.Save.autosave(g.s);}render();});box.appendChild(b)});root.appendChild(box);back();}
...
function render(){clear();if(!g)return;if(g.mode==='battle'){battle();if(modal)modal.style.display='none';return;}if(g.menu){if(modal)modal.style.display='none';if(g.menu.page==='root')rootMenu();else if(g.menu.page==='equipment')equipment();else if(g.menu.page==='jobs')jobs();else if(g.menu.page==='status')back();else if(['items','quests','bestiary'].includes(g.menu.page))simpleList();else back();return;} }
let sig='';setInterval(()=>{const s=[g.mode,g.menu?.page||'',g.menu?.index||0,g.shop?'shop':'',g.dialog?'dialog':'',g.battle?.turn||'',g.s?.player?.job||'',JSON.stringify(g.s?.player?.equipment||{}),JSON.stringify(g.s?.equipment||{})].join('|');if(s!==sig){sig=s;render();}},90);
