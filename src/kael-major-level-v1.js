'use strict';
(()=>{
  const level1=window.KaelLevel01;
  let sets=null;
  ...
  const makeSet=(level,paths)=>{
    ...
    const sync=()=>{set.ready=!!(walk.complete&&walk.naturalWidth);set.idleReady=!!(idle.complete&&idle.naturalWidth);set.attackReady=!!(attack.complete&&attack.naturalWidth);const p=window.__safehavenGame?.s?.player;if(p&&majorFromRank(p.rank||p.level||1)>=level&&(set.ready||set.idleReady||set.attackReady))window.KaelLevel01=set};
  ...
  const activate=major=>{const selected=latestAvailable(major);if(selected)window.KaelLevel01=selected;return selected};
  window.SafeHavenKaelSprites={sets,current,activate,majorFromRank,latestAvailable,level1};
})();